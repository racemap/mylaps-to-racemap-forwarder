	...
	if (aBufferToProcess.buffer.length > 0) {
		...
		while (CRLFPos > -1) {
			...
			const aTail: Buffer = Buffer.alloc(
				aBufferToProcess.buffer.length - CRLFPos - CRLF.length,
			);
...
export const printEnvVar = (
	envVar: { [name: string]: unknown },
	isPublic = true,
): void => {
	let name = "???";
	...
	console.log(
		now(),
		"Log:",
		`    |-> \x1b[35m${name}\x1b[0m: \x1b[36m${value || "???"}\x1b[0m`,
	);
};
...
//                       |      |> Time      |>  deviceNumber
//                       |> Transponder Id
export function myLapsLagacyPassingToRead(
	locationName: string,
	...
	if (passing.length > 38) {
		...
		const date = passingDetails.substring(
			passingDetails.length - 8,
			passingDetails.length - 2,
		);
		...
		const read: TimingRead = {
			...
			timestamp: moment
				.utc(`${date} ${time}`, "YYMMDD hh:mm:ss.SSS")
				.toISOString(),
...
export function myLapsMarkerToRead(
	locationName: string,
...
export function myLapsMarkerKeyToName(
	key: MyLapsMarkerShortKeys,
): MyLapsMarkerKeys {
	switch (key) {
		case "mt":
			return "markerType";
...
export function myLapsDeviceToObject(
	deviceAsString: string,
): MyLapsDevice | null {
	...
	if (
		deviceUpdate.deviceId != null &&
		deviceUpdate.deviceName != null &&
		deviceUpdate.deviceMac != null
	) {
		return deviceUpdate;
	}
	return null;
...
export function myLapsDeviceKeyToName(
	key: MyLapsDeviceShortKeys,
): MyLapsDeviceKeys {
	switch (key) {
		case "id":
			return "deviceId";
...
export function myLapsPassingToRead(
	timingId: string,
	timingName: string,
	passingAsString: string,
): TimingRead | null {
	...
	if (
		...
		return {
			...
			timestamp: moment
				.utc(`${passing.date}_${passing.time}`, "YYMMDD_hh:mm:ss.SSS")
				.toISOString(),
		};
	}
	return null;
...
export function myLapsPassingKeyToName(
	key: MyLapsPassingShortKeys,
): MyLapsPassingKeys {
	switch (key) {
		case "c":
			return "chipCode";
