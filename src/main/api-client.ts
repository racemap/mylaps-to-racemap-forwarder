...
import { error, log } from "./functions";
...
class APIClient {
	...
	async sendTimingReadsAsJSON(
		TimingReads: Array<TimingRead>,
	): Promise<Response> {
		return this._postJSON(
			"/services/trackping/api/v1/timing_input/pings",
			TimingReads,
		);
	}
	...
	async getTimingReads(query: {
		...
		return this._getJSON(
			withQuery("/services/trackping/api/v1/timing_output/pings", query),
		);
	}
}
