...
const logToFileSystem = (message: Buffer | string, fromClient = true) => {
	fs.appendFileSync(
		"./MyLapsInputAdapter.log",
		`${new Date().toISOString()} ${fromClient ? "» from" : "« to  "} client: ${message}\n`,
	);
};
...
class MyLapsForwarder extends BaseClass {
	...
	getForwarderState = (): ForwarderState => {
		const connections = Array.from(this._connections.values()).map(
			(socket) => ({
				id: socket.id,
	...
	_configureReceiverSocket = (
		...
		server.listen({ host: bindAddress, port: listenPort }, () => {
			info(
				`${RacemapMyLapsServerName} is listening on \x1b[32m${bindAddress}\x1b[0m:\x1b[35m${listenPort}\x1b[0m`,
			);
		});
		server.on("error", (err) => {
	...
	_onNewConnection = (socket: ExtendedSocket): void => {
		...
		socket.on("data", (data: Buffer) => {
			try {
				storeIncomingRawData(
					removeZeroBytesFromBuffer(data),
					socket.cache,
		...
		socket.sendData = (data: Array<string>) =>
	...
	_handleWelcomeMessage = (
		...
		if (
			parts.length === 3 &&
			(parts[1] === MyLapsFunctions.Ping || parts[1] === MyLapsFunctions.Pong)
		) {
			...
			refToSocket.sendData([
				RacemapMyLapsServerName,
				MyLapsFunctions.GetLocations,
			]);
			refToSocket.sendData([RacemapMyLapsServerName, MyLapsFunctions.GetInfo]);
		} else {
			warn(
				`${this.className}._handleWelcomeMessage`,
				"Unknown welcome message.",
				parts,
			);
		}
	};

	_createOrUpdateDevice = (
		location: MyLapsLocation,
		deviceUpdate: MyLapsDevice,
	): void => {
		if (deviceUpdate.deviceName != null) {
			const device: MyLapsDevice =
	...
	_createOrUpdateLocation = (
		...
		if (location == null) {
			success(
				`${this.className}._createOrUpdateLocation`,
	...
	_handleMessages = (
		...
		if (["v1.0", "v2.1"].includes(refToSocket.meta.version)) {
			...
			if (len > 1) {
				...
				switch (myLapsFunction) {
					case MyLapsFunctions.Pong: {
						const replied = refToSocket.sendData([
							RacemapMyLapsServerName,
							MyLapsFunctions.AckPong,
						]);
						...
						info(
							`${this.className}._handleMessages Pong received from client and ${replied ? "answered" : "not answered"}.`,
						);
						break;
					}
					...
					case MyLapsFunctions.Ping: {
						...
						this._createOrUpdateLocation(refToSocket, { locationName });
						info(
							`${this.className}._handleMessages Ping received from client and ${replied ? "answered" : "not answered"}.`,
						);
						break;
					}
					...
					case MyLapsFunctions.GetLocations: {
						...
						for (let i = 2; i < len - 1; i++) {
							...
							if (
								location.length === 2 &&
								location[0] ===
									MyLapsIdentifiers.LocationParameters.LocationName
							) {
					...
					case MyLapsFunctions.AckGetInfo: {
						if (len > 4) {
							...
							success(
								`${this.className}._handleMessages`,
								"AckGetInfo message received: ",
								locationName,
							);
						} else {
							warn(
								`${this.className}._handleMessages`,
								"AckGetInfo message with wrong part length received",
								parts.length,
								parts,
							);
						}
						...
						if (len > 4) {
							...
							if (counter > 0) {
								for (let i = 2; i < len - 2; i++) {
									...
									const read = myLapsPassingToRead(
										locationName,
										locationName,
										passing,
									);
									if (read != null) {
										...
										warn(
											`${this.className}._handleMessages`,
											"Passing message with missing keys received:",
											passing,
										);
									}
								}
					...
					case MyLapsFunctions.Store: {
						...
						if (len > 4) {
							...
							if (counter > 0) {
								for (let i = 2; i < len - 2; i++) {
									const read = myLapsLagacyPassingToRead(
										locationName,
										parts[i],
									);
									if (read != null) {
					...
					case MyLapsFunctions.Marker: {
						...
						if (len > 4) {
							...
							warn(
								`We received ${markers.length} Markers. But do not know what to do with them. So we drop them`,
							);
							refToSocket.sendData([
								RacemapMyLapsServerName,
								MyLapsFunctions.AckMarker,
								counter.toString(),
							]);
						}
					...
					default: {
						warn(
							"Message with unknown MyLaps function received:",
							myLapsFunction,
							parts,
						);
						break;
			...
			warn(
				`${this.className} - We do not handle messages because protocol version is not in [v1.0, v2.1]`,
			);
		}
	};

	async _pushNonlocatedReadToRacemap(
		timingReads: Array<TimingRead>,
	): Promise<void> {
